...
namespace VanguardLTE\Http\Controllers\Web\Frontend {
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function server_api(\Illuminate\Http\Request $request, $game)
        {
            // For security, only allow requests from localhost where the slots server is running.
            ...
            // It's best practice to store sensitive keys in your .env file.
            $expectedApiKey = env('SLOTS_API_KEY', 'a-very-secret-key');
            ...
            $game = trim($game);
            // Log::info($game);
            // Log::info( '\VanguardLTE\Games\\' . $game . '\Server');
            $object = '\VanguardLTE\Games\\' . $game . '\Server';
            if (!class_exists($object)) {
                echo '{"responseEvent":"error","responseType":"","serverResponse":"Game not found"}';
                exit();
            }

            $server = new $object();

            // Pass the request, game name, and the user ID to the server's get method.
